...
function QuizComponent({ topic, limit, clientType, questionType }: QuizComponentProps) {
  ...
  const handleSubmit = async () => {
    ...
    for (let i = 0; i < quiz.questions.length; i++) {
      ...
      const isCorrectForScoring = question.type === 'case_based' ? true : isAnswerCorrect(question, userAnswer);
      if (isCorrectForScoring) newScore++;
      answeredQuestions.push({
        question: question.question,
        userAnswer: userAnswer !== undefined ? JSON.stringify(userAnswer  CheckCircle,
  XCircle,
  Download,
  FileText,
  LayoutDashboard,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (mistakesToSave.length > 0) {
        ...
        const mistakesCollectionRef = collection(firestore, 'users', user  clientType: string;
  questionType: string;
}
